package dev.nikosg.poc.aitoolbox1.awesome;
...
public class RestTemplateHelper {
    ...
    public <T, R> T postForEntity(Class<T> clazz, String url, Map<String, String> headers, R body) throws JsonProcessingException {

        MultiValueMap<String, String> multiHeaders = new LinkedMultiValueMap<>();
        headers.forEach(multiHeaders::add);

        HttpEntity<R> request = new HttpEntity<>(body, multiHeaders);
        ResponseEntity<String> response = restTemplate.postForEntity(url, request, String.class, Map.of());
        JavaType javaType = objectMapper.getTypeFactory().constructType(clazz);
        return readValue(response, javaType);
    }
    ...
    public <T, R> T putForEntity(Class<T> clazz, String url, R body, Object... uriVariables) {
        ...
        ResponseEntity<String> response = restTemplate.exchange(url, PUT, request, String.class, uriVariables);
        JavaType javaType = objectMapper.getTypeFactory().constructType(clazz);
        return readValue(response, javaType);
    }
